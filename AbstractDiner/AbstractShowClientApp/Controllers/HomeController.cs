...
namespace AbstractShowClientApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            ViewBag.Snacks = APIClient.GetRequest<List<SnackViewModel>>("api/main/getsnacklist");
            return View();
        }

        [HttpPost]
        public void Create(int snack, int count, decimal sum)
        {
            if (count == 0 || sum == 0)
            {
                return;
        ...
        {
            SnackViewModel sn =
            APIClient.GetRequest<SnackViewModel>($"api/main/getsnack?snackId={snack}");
            return count * sn.Price;
        }
    }
}
